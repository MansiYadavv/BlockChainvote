...
const Admin = () => {
  ...
  const [candidateParty, setCandidateParty] = useState('');
  const [candidateManifesto, setCandidateManifesto] = useState('');
  const [electionDuration, setElectionDuration] = useState(60);
  
  useEffect(() => {
    // Check if contract is loaded before attempting to use it
    if (contract && account) {
      checkAdminStatus();
      loadElectionStatus();
  ...
  const checkAdminStatus = async () => {
    try {
      ...
      console.error("Error checking admin status:", error);
      setMessage({ type: 'danger', content: 'Error checking admin status: ' + error.message });
    }
  };
  ...
  const handleAddCandidate = async (e) => {
    ...
    try {
      console.log("Adding candidate:", candidateName, candidateParty, candidateManifesto);
      ...
      setMessage({ type: 'success', content: 'Candidate added successfully! Transaction hash: ' + tx.transactionHash });
  ...
  const handleStartElection = async () => {
    ...
    try {
      ...
      setMessage({ type: 'success', content: 'Election started successfully! Transaction hash: ' + tx.transactionHash });
      loadElectionStatus();
    } catch (error) {
      console.error("Error starting election:", error);
      setMessage({ 
  ...
  const handleEndElection = async () => {
    ...
    try {
      ...
      setMessage({ type: 'success', content: 'Election ended successfully! Transaction hash: ' + tx.transactionHash });
      loadElectionStatus();
    } catch (error) {
      console.error("Error ending election:", error);
      setMessage({ 
        type: 'danger', 
        content: 'Error ending election: ' + error.message
      });
    }
  };
        ...
        <div className="card-body">
          <form onSubmit={handleAddCandidate}>
            ...
            <button 
              type="submit" 
              className="btn btn-primary"
              disabled={electionStatus.isActive || !candidateName || !candidateParty}
            >
              Add Candidate
            </button>
          </form>
